...
object Request03_03Equipment {
  def main(args: Array[String]): Unit = {
    ...
    val sql03: DataFrame = sparkSession.sql(
      """ select
        |(case devicetype when 1 then "phone" when 2 then "PPC" else "other" end) devType,
    ...
    sql03.show()
    //默认输出
    sql03.write.json("data\\uselog\\requst03\\sql3")
  }
}
