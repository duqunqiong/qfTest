...
public class MapTask extends Task {
  ...
  public void run(final JobConf job, final TaskUmbilicalProtocol umbilical)
    ...
    this.umbilical = umbilical;//sss mr 负责与TaskTracker的通信，通过该对象可以获得必要的对象
    ...
    TaskReporter reporter = startReporter(umbilical);//sss mr 启动Reporter线程，用来和TaskTracker交互目前运行的状态
 
    boolean useNewApi = job.getUseNewMapper();
    initialize(job, getJobID(), reporter, useNewApi);//sss mr 用来初始化任务，主要是进行一些和任务输出相关的设置，比如创 建commiter，设置工作目录等
 ...
 private <T> T getSplitDetails(Path file, long offset) 
  ...
  private <KEY, VALUE> MapOutputCollector<KEY, VALUE>
    ...
    for (Class clazz : collectorClasses) {
      try {
        ...
        MapOutputCollector<KEY, VALUE> collector =//sss mr 创建圆形缓冲区对象 https://www.jb51.net/article/154381.htm
  ...
  static class NewTrackingRecordReader<K,V> 
    ...
    NewTrackingRecordReader(org.apache.hadoop.mapreduce.InputSplit split,
      ...
      this.real = inputFormat.createRecordReader(split, taskContext);//sss mr 在MapTask中使用TextInputFormat获取RecordReader
    ...
    public void initialize(org.apache.hadoop.mapreduce.InputSplit split,
      ...
      long bytesInPrev = getInputBytes(fsStats);
      real.initialize(split, context);//sss mr 在MapTask中初始化要切片的文件,包括文件偏移量等
      long bytesInCurr = getInputBytes(fsStats);
      fileInputByteCounter.increment(bytesInCurr - bytesInPrev);
    }
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
      long bytesInPrev = getInputBytes(fsStats);
      boolean result = real.nextKeyValue();//sss 判断是否有下一行数据
      long bytesInCurr = getInputBytes(fsStats);
      if (result) {
        inputRecordCounter.increment(1);
      }
  ...
  private class NewOutputCollector<K,V>
    ...
    NewOutputCollector(JobContext jobContext,
      ...
      collector = createSortingCollector(job, reporter);
      partitions = jobContext.getNumReduceTasks();//sss 设定分区数就是reduceTask的数量
      if (partitions > 1) {
    ...
    public void write(K key, V value) throws IOException, InterruptedException {
      collector.collect(key, value,
                        partitioner.getPartition(key, value, partitions)); //sss mr 调用系统默认HashPartition来获得分区
    }
  ...
  void runNewMapper(final JobConf job,
    ...
    TaskAttemptContext taskContext =
      new org.apache.hadoop.mapreduce.task.TaskAttemptContextImpl(job, 
                                                                  getTaskID(),
                                                                  reporter);
    // make a mapper sss mr maptask 通过反射来获取Mapper的地方
    org.apache.hadoop.mapreduce.Mapper<INKEY,INVALUE,OUTKEY,OUTVALUE> mapper =
      (org.apache.hadoop.mapreduce.Mapper<INKEY,INVALUE,OUTKEY,OUTVALUE>)
        ReflectionUtils.newInstance(taskContext.getMapperClass(), job);
    // make the input format sss mr maptask 通过反射来得到 InputFormat的地方
   ...
   //sss mr maptask 根据InputFormat对象创建RecordReader对象，默认是LineRecordReader
    ...
    try {//sss mr maptask 初始化，在默认情况下调用的是LineRecordReader的initialize方 法，主要是打开输入文件并且将文件指针指向文件头
      input.initialize(split, mapperContext);
      mapper.run(mapperContext); //sss mr 在MapTask中真正调用Mapper的地方
      mapPhase.complete();
      setPhase(TaskStatus.Phase.SORT);
      statusUpdate(umbilical);
      input.close();
  ...
  @InterfaceAudience.LimitedPrivate({"MapReduce"})
  @InterfaceStability.Unstable//sss mr maptask MapOutputBuffer 该类内部使用一个缓冲区暂时存储用户输出数据，当缓冲区使用率达到一定阈值后，再讲缓冲区中的数据写到磁盘上。Hadoop的这个缓冲区采用环形缓冲区：当缓冲区使用率达到一定的阈值后，便开始向磁盘上写入数据，同时生产者扔可以向不断增加的剩余空间中循环写入数据，进而达到读写并行(Map Task的collect阶段和spill阶段)，性能也比较高。
  public static class MapOutputBuffer<K extends Object, V extends Object>
    ...
    public void init(Context context
      ...
      final float spillper =
        job.getFloat(JobContext.MAP_SORT_SPILL_PERCENT, (float)0.8);//sss mr maptask 80%是环形缓冲区溢出百分比
      final int sortmb = job.getInt(JobContext.IO_SORT_MB, 100);//sss mr maptask 100M是默认排序缓冲区大小
     ...
     * storage to store one METADATA. sss mr 收集每次用户写入的数据数据
     */
    public synchronized void collect(K key, V value, final int partition
      ...
      if (partition < 0 || partition >= partitions) { //sss mr 如果分区数小于0
        throw new IOException("Illegal partition for " + key + " (" +
            partition + ")");
      }
      checkSpillException();
      bufferRemaining -= METASIZE;//默认是80M, 每次写入后要减去METASIZE的大小
      if (bufferRemaining <= 0) {//sss mr 如果超出最大缓存区的80%,那么就要开始溢写
          ...
          do {
            if (!spillInProgress) {
              ...
              if ((kvbend + METASIZE) % kvbuffer.length !=
                ...
                continue;
              } else if (bufsoftlimit && kvindex != kvend) {//sss mr maptask 先检查kvoffsets数组的有效容量是否超过io.sort.spill.percent，默认0.8，如果超过则唤醒spill线程写到临时文件中( startSpill()方法完成)
      ...
      try {
        ...
        // write accounting info sss mr 在环形缓冲区中写入元数据信息
        kvmeta.put(kvindex + PARTITION, partition); //元数据信息中分区的偏移量
        kvmeta.put(kvindex + KEYSTART, keystart);//元数据信息中key的起始位置
        kvmeta.put(kvindex + VALSTART, valstart);//元数据信息中value的起始位置
        kvmeta.put(kvindex + VALLEN, distanceTo(valstart, valend));//元数据信息中心value的长度
        // advance kvindex
    ...
    public class Buffer extends OutputStream {
      ...
      public void write(byte b[], int off, int len)
        ...
        // here, we know that we have sufficient(充足的) space to write
    ...
    public void flush() throws IOException, ClassNotFoundException,
      ...
      kvbuffer = null;
      mergeParts();//sss mr maptask 最后调用mergeParts()方法合并所有spill文件 ccc mergeParts
      Path outputPath = mapOutputFile.getOutputFile();
      fileOutputByteCounter.increment(rfs.getFileStatus(outputPath).getLen());
    }

    public void close() { }
//sss mr maptask SpillThread这个线程在MapOutputBuffer初始化时被启动，它会一直监视着环形缓冲区当环形缓冲区超过一定值(80%)的时候将会调用sortAndSpill方法，讲环形缓冲区中已经计算完毕的数据写到本地文件中
    ...
    private void sortAndSpill() throws IOException, ClassNotFoundException,
      ...
      try {
        ...
        for (int i = 0; i < partitions; ++i) {//sss mr 遍历每个分区,进行数据处理
          ...
          try {
            ...
            } else {//sss 如果combine不为空,则说明设置了CombineClass,那么在溢写的时候调用combine方法
              int spstart = spindex;
              while (spindex < mend &&
